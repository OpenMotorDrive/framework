...
static void begin_flash_from_path(uint8_t uavcan_idx, uint8_t source_node_id, struct uavcan_protocol_file_Path_s path) {
    ...
    worker_thread_add_timer_task(&WT, &flash_state.read_timeout_task, read_request_response_timeout, NULL, 500, false);
...
static bool check_and_start_boot_timer(void) {
    if (app_info.shared_app_parameters && app_info.shared_app_parameters->boot_delay_sec != 0) {
        start_boot_timer((uint32_t)app_info.shared_app_parameters->boot_delay_sec * 1000UL);
        return true;
    }
    return false;
}
...
static void restart_req_handler(size_t msg_size, const void* buf, void* ctx) {
    ...
    if ((msg->magic_number == UAVCAN_PROTOCOL_RESTARTNODE_REQ_MAGIC_NUMBER) && system_get_restart_allowed()) {
        res.ok = true;
        worker_thread_add_timer_task(&WT, &delayed_restart_task, delayed_restart_func, NULL, 1000, false);
    }
