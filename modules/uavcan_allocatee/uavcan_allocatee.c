...
static void allocation_start_request_timer(struct allocatee_instance_s* instance) {
    ...
    worker_thread_remove_timer_task(&WT, &instance->request_transmit_task);
    worker_thread_add_timer_task(&WT, &instance->request_transmit_task, allocation_timer_expired, instance, MS2US(request_delay_ms), false);
}
...
static float getRandomFloat(void) {
    ...
    if (state == 0) {
        ...
        // TODO: does this perform well with the system timer running at 10KHz?
